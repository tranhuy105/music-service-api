...
public class PlaylistDao implements PlaylistRepository {
    ...
    @Override
    public List<Playlist> findSystemGeneratedFeaturedPlaylistByGenreName(String genreName) {
        String sql =
                "SELECT " +
                    "p.id AS playlist_id, " +
                ...
                "WHERE MATCH(name) AGAINST (? IN NATURAL LANGUAGE MODE) AND p.user_id IS NULL " +
                "GROUP BY p.id, p.name, p.cover_url";
        return jdbcTemplate.query(sql, new PlaylistSummaryRowMapper(), genreName);
    }
